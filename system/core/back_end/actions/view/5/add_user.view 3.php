            ...
            if ((THEME_LANG_BCK == $action_lang_auth || $action_lang_auth == '_all') && (THEME_FOLDER_BCK == $action_folder_auth || $action_folder_auth == '_all') && (TPL_AUTH_BCK == $action_tpl_auth || $action_tpl_auth == '_all')) {
                     	    ...
                     	    if ($unauthorized_lang === false) {
                     	        if (strtolower(trim(SESSION_PAGE_LANG_BCK)) !== strtolower(trim($abbr_language))) {
                     	        	...
                     	        	if (isset($_SESSION['user_theme_lang_bck']) && $_SESSION['user_theme_lang_bck'] !== '') {
                     	        		$content2 .= Fxr_lib\Template::loadSubTemplates('5_add_user' . DS . 'add_page_select_option_selected', 1, '', array('user_lang_timezone_bck' => strtolower(trim($_SESSION['user_theme_lang_bck']))));
                     	        	} else {
                     		...
                     		foreach ($timezones as $abbr_timezone => $name_timezone) {
                     		    ...
                     		    foreach ($name_timezone as $abbr_timezone2 => $name_timezone2) {
                     		        if (Fxr_lib\Database::loadQuery('select', "SELECT user_timezone_bck FROM " . DB_PREFIX . "users_bck", '', '') !== $name_timezone2) {
                     		        	...
                     		        	$content3 .= Fxr_lib\Template::loadSubTemplates('5_add_user' . DS . 'add_page_select_option_selected', 1, '', array('user_lang_timezone_bck' => $_SESSION['user_timezone_bck']));
                     		            } else {
                    ...
                    if (defined("GET_MODIFIED_ID") && GET_MODIFIED_ID !== '') {
                        ...
                        $content .= Fxr_lib\Template::loadSubTemplates('5_add_user' . DS . 'add_page_form_fields_modified', '', Fxr_lib\Database::loadQuery('select', "SELECT user_id_bck, user_login_bck, user_password_bck, user_name_bck, user_email_bck, user_theme_bck, user_level_access_bck, user_lang_bck FROM " . DB_PREFIX . "users_bck WHERE user_id_bck = :user_id_bck", '+', array(
                            ...
                            (isset($_SESSION['user_theme_folder_bck']) && $_SESSION['user_theme_folder_bck'] !== '') ? 'user_theme_folder_bck' : '' => $_SESSION['user_theme_folder_bck'],
                            (isset($_SESSION['user_level_access_bck']) && $_SESSION['user_level_access_bck'] !== '') ? 'user_level_access_bck' : '' => $_SESSION['user_level_access_bck'],
                            
                            'user_theme_lang_bck' => Fxr_lib\Database::loadQuery('select', "SELECT user_lang_bck FROM " . DB_PREFIX . "users_bck WHERE user_id_bck = :user_id_bck", '', array(
                                ':user_id_bck' => array(
                                    GET_MODIFIED_ID => 'i'
                                )
            ...
            if ((THEME_LANG_BCK == $action_lang_auth || $action_lang_auth == '_all') && (THEME_FOLDER_BCK == $action_folder_auth || $action_folder_auth == '_all') && (TPL_AUTH_BCK == $action_tpl_auth || $action_tpl_auth == '_all')) {
                ...
                if (isset($_GET['fail'])) {
                    ...
                    if (count($fields_without_nulls) == 0) {
                        ...
                        $content .= Fxr_lib\Template::loadSubTemplates('2_add_page' . DS . 'add_page_error_begin_one', 1, '', '');
                    } else {
                        $content .= Fxr_lib\Template::loadSubTemplates('2_add_page' . DS . 'add_page_error_begin_many', 1, '', '');
                    }
                    ...
                    $content .= Fxr_lib\Template::loadSubTemplates('2_add_page' . DS . 'add_page_error_end', 1, '', '');
                }
